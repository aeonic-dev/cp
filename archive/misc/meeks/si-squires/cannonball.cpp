...
void solve() {
    ...
    vec<dpt> pts(n + 2);
    rep(i, 0, n) cin >> pts[1 + i];
    pts[0] = s;
    pts[n + 1] = e;

    vec<vec<double>> edges(n + 2, vec<double>(n + 2, DBL_MAX));
    rep(i, 1, n + 2) edges[0][i] = s.distance(pts[i]) / 5;
    rep(i, 1, n + 1) {
        rep(j, 1, n + 2) {
            double d = pts[i].distance(pts[j]);
            double c = min(d, 10 + abs(d - 50)) / 5;
            edges[i][j] = c;
        }
    }
    
    rep(k, 0, n + 2) rep(i, 0, n + 2) rep(j, 0, n + 2) edges[i][j] = min(edges[i][j], edges[i][k] + edges[k][j]);
    cout << edges[0][n + 1] << endl;
}
