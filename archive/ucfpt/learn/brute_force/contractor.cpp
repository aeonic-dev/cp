...
ll solve(int n, int d, vec<vll> &dp, vpi &jobs, int next, int days) {
    if (d <= 0 || next >= n) return 0;

    auto &res = dp[next][days];
    if (res == -1) {
        res = solve(n, d, dp, jobs, next + 1, days);
        if (days >= jobs[next].first) res = max(res,
                jobs[next].second + solve(n, d, dp, jobs, next + 1, days - jobs[next].first)
        );
    }
...
void solve() {
    ...
    vec<vll> dp(n, vll(d + 1, -1));
    cout << solve(n, d, dp, jobs, 0, d);
}
