...
ul index(int cols, int r, int c) {
    return cols * r + c;
}

void add_edges(ffa<ul, ll, LONG_LONG_MAX> &network, int r, int c, int capacity) {
...
void solve() {
    ...
    ffa<ul, ll, LONG_LONG_MAX> network(2 + rows * cols);
    ul source = rows * cols, sink = source + 1;
