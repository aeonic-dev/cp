...
ll count(int x, int d, vi &v, vec<vll> &dp, int next, int target) {
    if (target < 0) return 0;
    if (next >= d) return target == 0;

    auto &res = dp[target][next];
    if (res == -1) {
        res = count(x, d, v, dp, next + 1, target) % mod;
        res += count(x, d, v, dp, next, target - v[next]) % mod;
        res %= mod;
    }
...
void solve() {
    ...
    vi v(d);
    cin >> v;
    reverse(all(v)); // sort descending to eliminate configurations faster

    vec<vll> dp(x + 1, vll(d, -1));
    cout << count(x, d, v, dp, 0, x) << endl;
}
