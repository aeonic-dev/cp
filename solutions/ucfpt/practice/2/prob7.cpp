...
string lower(string s) {
    string ret;
    travis(x, s) if ((x >= 'a' && x <= 'z') || (x >= 'A' && x <= 'Z')) ret.push_back(tolower(x));
    return ret;
}
...
void solve() {
    ...
    while (getline(cin, line)) {
        rep(i, 0, line.size()) {
            ...
            if (c == '#' && (last == ' ' || last == '\n')) {
                string s;
                while (c = line[++i], (c >= 'a' && c <= 'z') || (c >= 'A' && c <= 'Z')) {
                    s += c;
                }

                string su = lower(s);
                if (tags.find(su) == tags.end()) tags[su] = 1;
                else tags[su]++;
            }
    ...
    vec<pair<int, string>> sorted;
    travis(x, tags) {
        sorted.emplace_back(x.second, x.first);
    }

    sort(all(sorted), [](pair<int, string> a, pair<int, string> b) {
