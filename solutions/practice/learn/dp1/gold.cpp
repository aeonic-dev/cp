...
ll solve(int n, vi &v, vec<vll> &dp, int left, int right) {
    ...
    res = max(
            v[left] - solve(n, v, dp, left + 1, right),
            v[right] - solve(n, v, dp, left, right - 1)
    );
...
void solve() {
    ...
    // left, right, me
    vec<vll> dp(n, vll(n, LLONG_MAX));
    cout << solve(n, v, dp, 0, n - 1) << endl;
}
